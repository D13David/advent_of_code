...
rebootCode* rebootSequence;
int numRebootCodes;

int* sortedCoords;
int numSortedCoords;

void getCoordStreams(int** xstream, int** ystream, int** zstream)
{
    *xstream = sortedCoords;
    *ystream = *xstream + _countof(data) * 2;
    *zstream = *ystream + _countof(data) * 2;
}

int firstIndexOf(int* stream, int value)
{
    for (int i = 0; i < _countof(data) * 2; ++i)
    {
        if (stream[i] == value)
            return i;
    }
...
void initialize()
{
    sortedCoords = (int*)malloc(_countof(data) * 6 * sizeof(int));
    assert(sortedCoords != NULL && "allocation failed");

    int* xstream, * ystream, * zstream;
    getCoordStreams(&xstream, &ystream, &zstream);

    rebootSequence = (rebootCode*)malloc(_countof(data) * sizeof(rebootCode));
    assert(rebootSequence != NULL && "allocation failed");
    for (int i = 0; i < _countof(data); ++i)
    {
        char isOn[4] = { 0 };
        vec3 min, max;
        int read = sscanf(data[i], "%s x=%d..%d,y=%d..%d,z=%d..%d", &isOn, &min.x, &max.x, &min.y, &max.y, &min.z, &max.z);
        assert(read == 7);
        ...
        * xstream++ = min.x; *xstream++ = (max.x + 1);
        *ystream++ = min.y; *ystream++ = (max.y + 1);
        *zstream++ = min.z; *zstream++ = (max.z + 1);

        struct rebootCode_t code =
        {
            .min = min,
            .max = max,
            .on = !strcmp(isOn, "on")
        };
        rebootSequence[numRebootCodes++] = code;
    }

    numSortedCoords = xstream - sortedCoords;

    getCoordStreams(&xstream, &ystream, &zstream);
    qsort(xstream, numSortedCoords, sizeof(int), compare);
    qsort(ystream, numSortedCoords, sizeof(int), compare);
    qsort(zstream, numSortedCoords, sizeof(int), compare);
}
...
int main()
{
    ...
    int size = numSortedCoords * numSortedCoords * numSortedCoords;
    unsigned char* buffer = (unsigned char*)malloc(size * sizeof(unsigned char));
    assert(buffer != NULL && "allocation failed...");
    memset(buffer, 0, size * sizeof(unsigned char));

    for (int i = 0; i < numRebootCodes; ++i)
    {
        rebootCode code = rebootSequence[i];
        int rangeXmin = firstIndexOf(xstream, code.min.x); int rangeXmax = firstIndexOf(xstream, code.max.x + 1);
        int rangeYmin = firstIndexOf(ystream, code.min.y); int rangeYmax = firstIndexOf(ystream, code.max.y + 1);
        int rangeZmin = firstIndexOf(zstream, code.min.z); int rangeZmax = firstIndexOf(zstream, code.max.z + 1);
        assert(rangeXmin >= 0 && rangeXmax >= 0 &&
            rangeYmin >= 0 && rangeYmax >= 0 &&
            rangeZmin >= 0 && rangeZmax >= 0);
        for (int z = rangeZmin; z < rangeZmax; ++z)
            for (int y = rangeYmin; y < rangeYmax; ++y)
                for (int x = rangeXmin; x < rangeXmax; ++x)
                    buffer[(z * numSortedCoords * numSortedCoords) + (y * numSortedCoords) + x] = code.on;
    }

    __int64 count = 0;
    for (int z = 0; z < numSortedCoords - 1; ++z)
        for (int y = 0; y < numSortedCoords - 1; ++y)
            for (int x = 0; x < numSortedCoords - 1; ++x)
            {
                if (buffer[(z * numSortedCoords * numSortedCoords) + (y * numSortedCoords) + x] == 0)
                    continue;

                __int64 w = (__int64)xstream[x + 1] - xstream[x];
                __int64 h = (__int64)ystream[y + 1] - ystream[y];
                __int64 d = (__int64)zstream[z + 1] - zstream[z];
                count += w * h * d;
            }
