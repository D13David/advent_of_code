main() {
  extrn data, chunks, part2;
  auto i, j, len, sum, doeval;
...
next:
  ...
  while (j < len) {
    if (part2) {
        if (compare(data[i], "do()", j)) {
            j =+ 4;
            doeval = 1;
        } else if (compare(data[i], "don't()", j)) {
            j =+ 7;
            doeval = 0;
        }
    }
    if (doeval & compare(data[i], "mul", j)) {
      j =+ 3;
      sum =+ evalexpr(data[i], &j);
    }
    ++j;
  }
  if (i < chunks) {
...
evalexpr(str, offset) {
  auto c, value1, value2;
  
  if (!expect(str, '(', *offset)) {
    return (0);
  }
  
  *offset =+ 1; value1 = parsenum(str, offset);
  
  if (!expect(str, ',', *offset)) {
    return (0);
  }
  
  *offset =+ 1; value2 = parsenum(str, offset);
...
expect(str, value, offset) {
    extrn char;
    return (char(str, offset) == value);
}
...
compare(str, needle, offset) {
  ...
  i = 0;
  while ((c1 = char(needle, i)) != '*0') {
    c2 = char(str, i+offset);
    if (c1 != c2) {
      return (0);
    }
    ++i;
